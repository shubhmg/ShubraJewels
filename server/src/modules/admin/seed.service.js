...
import GalleryItem from '../gallery/gallery.model.js';
...
const CONTENT_MODELS = [Category, Collection, Product, Banner, Video, Review, GalleryItem];

/** Delete every content document (keeps orders, visits, settings, admin). */
export async function clearContent() {
  const removed = {};
  for (const M of CONTENT_MODELS) {
    const r = await M.deleteMany({});
    removed[M.modelName] = r.deletedCount;
  }
  return removed;
}

/**
 * Insert starter content. By default only fills EMPTY collections (safe).
 * `force: true` wipes content first, then reseeds.
 */
export async function seedContent({ force = false } = {}) {
  ...
  if ((await Category.countDocuments()) === 0) {
    const docs = await Category.insertMany(CATEGORIES.map((c, i) => ({ ...c, slug: slugify(c.name), order: i, isActive: true })));
    catMap = Object.fromEntries(docs.map((d) => [d.name, d._id]));
    summary.categories = docs.length;
  } else (await Category.find().lean()).forEach((d) => { catMap[d.name] = d._id; });

  // Collections
  let colMap = {};
  if ((await Collection.countDocuments()) === 0) {
    const docs = await Collection.insertMany(COLLECTIONS.map((c, i) => ({ ...c, slug: slugify(c.name), order: i, isActive: true })));
    colMap = Object.fromEntries(docs.map((d) => [d.name, d._id]));
    summary.collections = docs.length;
  } else (await Collection.find().lean()).forEach((d) => { colMap[d.name] = d._id; });

  // Products
  if ((await Product.countDocuments()) === 0) {
    const docs = PRODUCTS.map((p, i) => ({
      name: p.name, hindiName: p.hindiName, slug: slugify(p.name), sku: `SJ-${String(i + 1).padStart(3, '0')}`,
      story: p.story, description: p.story, price: p.price, mrp: p.mrp || 0, isOnSale: (p.mrp || 0) > p.price,
      categoryId: catMap[p.category] || null, collectionIds: (p.collections || []).map((n) => colMap[n]).filter(Boolean),
      images: p.images, material: p.material || '', weight: p.weight || '', tags: p.tags || [],
      stockQty: 25, inStock: true, isNewArrival: !!p.isNew, isBestseller: !!p.isBestseller,
      ratingAvg: 4.6 + Math.random() * 0.4, ratingCount: 40 + Math.floor(Math.random() * 400), order: i, isActive: true,
    }));
  ...
  if ((await Banner.countDocuments()) === 0) {
    await Banner.insertMany([
      { placement: 'topStrip', text: 'Free shipping across Delhi 🚚  •  Korean earrings FREE on orders above ₹1499', hindiText: 'दिल्ली में फ्री शिपिंग', order: 0, isActive: true },
      { placement: 'offer', text: 'Korean Earrings FREE', hindiText: 'कोरियन इयररिंग्स मुफ़्त', subtext: 'On every order above ₹1499', ctaLabel: 'Shop Now', ctaLink: '/products', bgColor: '#7B1E2B', image: IMG.b, order: 1, isActive: true },
      { placement: 'offer', text: 'Delhi? Ships FREE', hindiText: 'दिल्ली में फ्री डिलीवरी', subtext: 'Same-week delivery inside Delhi NCR', ctaLabel: 'Explore', ctaLink: '/products', bgColor: '#C9A84C', image: IMG.c, order: 2, isActive: true },
    ]);
    summary.banners = 3;
  }
  ...
  if ((await Video.countDocuments()) === 0) {
    await Video.insertMany([
      { title: 'हर झुमका एक कहानी', caption: 'The making of a Shubra jhumka', src: 'https://cdn.coverr.co/videos/coverr-golden-jewellery-1573/1080p.mp4', poster: IMG.a, isHero: true, order: 0, isActive: true },
    ]);
    summary.videos = 1;
  }
