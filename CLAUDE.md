...
- **Delhivery courier integration:** `server/src/utils/delhivery.js` — client for Delhivery One API (base `track.delhivery.com`, staging `staging-express.delhivery.com`; auth header `Authorization: Token <token>`). Endpoints wrapped: create shipment (`POST /api/cmu/create.json`, form-urlencoded `format=json&data=<json>`), serviceability (`GET /c/api/pin-codes/json/?filter_codes=`), track (`GET /api/v1/packages/json/?waybill=`), cancel (`POST /api/p/edit {waybill,cancellation:'true'}`), packing-slip label (`GET /api/p/packing_slip?wbns=&pdf=true`). All reads best-effort (return `{ok,...,error}`, never throw); shipment creation surfaces errors to the admin.
  - **Config (SECRET):** `Setting.delhivery` = `{ enabled, token (secret), staging, policy: all|cod|prepaid|manual, pickupName (exact registered warehouse, case-sensitive), pickupPhone/Address/City/State/Pin, defaultWeightGrams (per-item fallback ×qty), sellerName, productDesc }`. The API **token is stripped** from public `GET /settings` (and from the `PATCH /settings` response) like `notifications`; the admin reads it via `GET /settings/admin`. `PATCH /settings` deep-merges `delhivery` so partial saves keep the token.
  - **Policy** (`shouldAutoShip`/frontend `policyApplies`): decides which orders route through Delhivery when shipped — `all` / `cod` (COD only) / `prepaid` (online-paid only) / `manual` (per-order button). `orderPaymentMode(order)` = Prepaid if paid or non-COD method, else COD; COD collect amount = `total − advancePaid`.
  - **Order.shipment** subdoc: `{ provider: manual|delhivery, waybill, mode, codAmount, weightGrams, status, statusDetail, labelUrl, bookedAt, lastSyncedAt }`.
  - **Order routes:** `POST /orders/:id/ship-delhivery {weight?}` (books waybill, sets shipment + status shipped + auto-fills `tracking.message` with the Delhivery track link + sends shipped email + fetches label in background), `POST /orders/:id/sync-shipment` (refresh live status; auto-advances to delivered when courier confirms), `POST /orders/:id/cancel-shipment` (cancels AWB, order status unchanged), `GET /orders/:id/label` (packing-slip PDF link), `GET /orders/delhivery/serviceability?pin=`. Test: `POST /settings/test-delhivery` (validates token via serviceability). Customer tracking URL = `delhivery.com/track/package/<waybill>`.
  - **Admin UI:** Settings → *Payments & Shipping* tab has a **"Delhivery Shipping (automated)"** section (enable, policy, token, pickup warehouse + address, default weight, staging toggle, Test connection).

- **Shiprocket courier integration:** `server/src/utils/shiprocket.js` — client for the Shiprocket aggregator (base `apiv2.shiprocket.in/v1/external`). Auth is a **login API** (`POST /auth/login {email,password}` → JWT valid ~10 days) — `ensureToken(settingDoc)` caches the JWT on `Setting.shiprocket.token`+`tokenExpiry` and re-logs in when expired (persists via the passed Mongoose doc). Booking is a 3-step dance: `POST /orders/create/adhoc` → `POST /courier/assign/awb` (Shiprocket picks the recommended courier) → `POST /courier/generate/label` (+ `generate/pickup`). Also serviceability (`GET /courier/serviceability/`), track (`GET /courier/track/awb/:awb`), cancel (`POST /orders/cancel {ids:[srOrderId]}`).
  - **Config (SECRET):** `Setting.shiprocket` = `{ enabled, email (secret), password (secret), token+tokenExpiry (cached JWT, secret), policy, pickupLocation (nickname, exact), pickupPin (for serviceability), defaultWeightKg (×qty), length/breadth/height (cm) }`. Stripped from public `GET /settings` + `PATCH` response like `delhivery`. `PATCH /settings` deep-merges and **clears the cached JWT if email/password change**. Test: `POST /settings/test-shiprocket` (logs in).
  - Shiprocket uses **kg**; Delhivery uses **grams**. `orderPaymentMode`/`shouldAutoShip`/`policy` identical to Delhivery. `shiprocket.autoPickup` (default **false**) gates the `generate/pickup` call so test bookings don't summon a courier; turn ON for live orders.
  - **Status webhook:** `POST /api/orders/shiprocket-webhook` (public, verified against `shiprocket.webhookToken` via the `x-api-key` header — no token configured = webhook disabled/401). Finds the order by `shipment.waybill`, updates `shipment.status/statusDetail/lastSyncedAt`; on a Delivered status (excluding "RTO Delivered") auto-advances shipped→delivered + COD→paid + `reconcileOrderStock`. Unknown AWBs are ACKed with 200 (no retries). Settings UI shows the webhook URL to copy + token field + Generate button; configure in Shiprocket → Settings → API → Webhooks.
...
- **Shared courier plumbing** (`Order.shipment` + order routes):
  - `Order.shipment` = `{ provider: manual|delhivery|shiprocket, waybill (AWB), shipmentId (SR label/pickup), srOrderId (SR cancel), courierName, trackingUrl (provider-specific), mode, codAmount, weightGrams, status, statusDetail, labelUrl, bookedAt, lastSyncedAt }`.
  - Booking routes `POST /orders/:id/ship-delhivery {weight? grams}` and `POST /orders/:id/ship-shiprocket {weight? kg}` both funnel through `applyBooking()` (stamps shipment, marks Shipped, auto-fills `tracking.message` + track link, sends shipped email). **sync/cancel/label routes are now provider-agnostic** — they dispatch on `order.shipment.provider` (`sync-shipment` auto-advances to delivered when the courier confirms; `cancel-shipment` accepts `{revert}` — `revert:true` (one-click "Cancel & reset") wipes the shipment + tracking and steps a Shipped order back to Confirmed for re-booking; `GET /:id/label`). Serviceability probes: `GET /orders/delhivery/serviceability?pin=` and `GET /orders/shiprocket/serviceability?pin=&weight=&cod=` (needs `shiprocket.pickupPin`).
  - **`pages/admin/Orders.jsx` `ShipModal`** builds a tab per enabled courier (Delhivery / Shiprocket) + a **Manual note** tab; preselects the courier whose policy auto-routes the order (else first ready courier, else manual). Each courier tab shows PIN serviceability (Delhivery: COD/prepaid flags; Shiprocket: courier count + cheapest rate) + a weight field (g vs kg) and books via the matching endpoint; incomplete setup shows an amber "finish setup" hint instead of hiding. Order detail shows one **courier card** (provider name + courierName, AWB link via `shipment.trackingUrl`, live status, COD collect, Track/Label/Sync/Cancel-AWB). Orders page loads `/settings/admin` once into `delCfg` + `srCfg` (enabled/policy/ready). `Field` gained a `password` type (`components/admin/AdminUI.jsx`).
